...
图结构:
START → fetch_snapshot → retrieve_rag_context → behavior_analysis → action_reasoning → merge_output → END
...
def build_orchestrator() -> StateGraph:
    ...
    # 线性边
    builder.add_edge(START, "fetch_sanpshot")
    builder.add_edge("fetch_snapshot", "retrieve_rag_context")
    builder.add_edge("retrieve_rag_context", "behavior_analysis")

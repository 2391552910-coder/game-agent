...
class TestIntentInferenceNode:
    ...
    async def test_no_session_events_returns_low_confidence(self):
        ...
        mock_chain.ainvoke = AsyncMock(return_value=InferredIntent(
            session_summary="无行为数据",
            intent_confidence="low",
        ))

        with patch("src.core.agents.decision_nodes.get_llm", AsyncMock(return_value=mock_llm)), \
             ...
             patch("langchain_core.prompts.ChatPromptTemplate.from_messages", return_value=MagicMock(__or__=MagicMock(return_value=mock_chain))):
    ...
    async def test_with_session_events(self):
        ...
        with patch("src.core.agents.decision_nodes.get_llm", AsyncMock(return_value=MagicMock())), \
             patch("src.core.agents.decision_nodes._load_session_events", AsyncMock(return_value=[{"event_type": "move"}])), \
             patch("src.core.agents.decision_nodes._load_recent_intents", AsyncMock(return_value=[])), \
             patch("langchain_core.prompts.ChatPromptTemplate.from_messages", return_value=MagicMock(__or__=MagicMock(return_value=mock_chain))):
            result = await intent_inference_node(state)
    ...
    async def test_llm_failure_returns_error(self):
        ...
        with patch("src.core.agents.decision_nodes.get_llm", AsyncMock(side_effect=Exception("LLM unavailable"))), \
             patch("src.core.agents.decision_nodes._load_session_events", AsyncMock(return_value=[])), \
             patch("src.core.agents.decision_nodes._load_recent_intents", AsyncMock(return_value=[])):
            result = await intent_inference_node(state)
...
class TestGoalEvaluationNode:
    ...
    async def test_switch_goal_high_cost(self):
        ...
        with patch("src.core.agents.decision_nodes.get_llm", AsyncMock(return_value=MagicMock())), \
             patch("src.core.agents.decision_nodes._load_last_intent", AsyncMock(return_value={"current_goal": "PVP排名"})), \
             patch("langchain_core.prompts.ChatPromptTemplate.from_messages", return_value=MagicMock(__or__=MagicMock(return_value=mock_chain))):
            result = await goal_evaluation_node(state)
    ...
    async def test_llm_failure_fallback_to_new(self):
        ...
        with patch("src.core.agents.decision_nodes.get_llm", AsyncMock(side_effect=Exception("fail"))), \
             patch("src.core.agents.decision_nodes._load_last_intent", AsyncMock(return_value=None)):
            result = await goal_evaluation_node(state)
...
class TestMemoryUpdateNode:
    ...
    async def test_upsert_called(self):
        ...
        with patch("src.core.agents.decision_nodes._upsert_player_memory", AsyncMock()) as mock_upsert, \
    ...
    async def test_upsert_failure_returns_error(self):
        ...
        with patch("src.core.agents.decision_nodes._upsert_player_memory", AsyncMock(side_effect=Exception("DB error"))), \
    ...
    async def test_save_intent_called_with_correct_args(self):
        ...
        with patch("src.core.agents.decision_nodes._upsert_player_memory", AsyncMock()), \
             patch("src.core.agents.decision_nodes._save_player_intent", AsyncMock()) as mock_save:
...
class TestUpdateBehaviorProfile:
    ...
    def test_sliding_average(self):
        ...
        existing = {"avg_spend_per_session": 100.0, "avg_session_minutes": 30.0, "spend_tendency": "low", "preferred_content": []}
...
class TestUpdateGoalHistory:
    ...
    def test_second_occurrence_written(self):
        ...
        result = _update_goal_history(pre_history, {"decision": "continue", "suggested_goal_type": "pvp_rank"}, analysis_count=2)
